...
- `parse_execute_md(project_name, iteration_num, max_iterations) -> str`
...
while iterations_used < max_iterations:
    if done.md exists:
        delete done.md
        check if all tasks in tasks.json are complete -> if so, write summary.md and exit
        check if any task hit max_attempts -> if so, write summary.md and halt
...
- `max_attempts`: if `attempts` reaches this value, halt execution entirely and write `summary.md`
...
- All tasks in `tasks.json` are `completed` → write `summary.md`, exit cleanly
- A task's `attempts` reaches `max_attempts` → halt, write `summary.md` with reason
- `--iterations` limit is reached → halt, write `summary.md` documenting progress
...
| `max_attempts` breach | Halt entire execution, write `summary.md` |
