...
class TestCmdInterviewCoreFunction:
    ...
    def test_calls_assert_project_exists(self):
        ...
        with (
            patch("ralph.cli._assert_project_exists") as mock_assert,
            patch("ralph.cli.Runner") as MockRunner,
            patch("ralph.cli.parse_questions_md", return_value="q-prompt"),
            patch("ralph.cli.parse_generate_tasks_md", return_value="a-prompt"),
            patch("ralph.cli.get_rounds", return_value=1),
    ...
    def test_calls_run_interview_loop(self):
        ...
        with (
            patch("ralph.cli._assert_project_exists"),
            patch("ralph.cli.Runner") as MockRunner,
            patch("ralph.cli.parse_questions_md", return_value="q"),
            patch("ralph.cli.parse_generate_tasks_md", return_value="a"),
            patch("ralph.cli.get_rounds", return_value=1),
            patch("ralph.cli.get_verbose", return_value=False),
        ):
    ...
    def test_uses_cli_rounds_when_provided(self):
        ...
        with (
            patch("ralph.cli._assert_project_exists"),
            patch("ralph.cli.Runner") as MockRunner,
            patch("ralph.cli.parse_questions_md", return_value="q") as mock_parse_q,
            patch("ralph.cli.parse_generate_tasks_md", return_value="a"),
            patch("ralph.cli.get_rounds", return_value=99),
            patch("ralph.cli.get_verbose", return_value=False),
        ):
            MockRunner.return_value.run_interview_loop = MagicMock()
    ...
    def test_uses_settings_rounds_when_not_provided(self):
        ...
        with (
            ...
            patch("ralph.cli.parse_questions_md", return_value="q") as mock_parse_q,
            patch("ralph.cli.parse_generate_tasks_md", return_value="a"),
            patch("ralph.cli.get_rounds", return_value=3),
            patch("ralph.cli.get_verbose", return_value=False),
        ):
            MockRunner.return_value.run_interview_loop = MagicMock()
    ...
    def test_question_prompts_list_has_one_entry_per_round(self):
        ...
        with (
            ...
            patch("ralph.cli.parse_questions_md", return_value="q-prompt"),
            patch("ralph.cli.parse_generate_tasks_md", return_value="a-prompt"),
            patch("ralph.cli.get_rounds", return_value=2),
            patch("ralph.cli.get_verbose", return_value=False),
        ):
            mock_runner = MockRunner.return_value
            cmd_interview(_make_args(rounds=2))
...
class TestCmdInterviewFailcases:
    ...
    def test_run_interview_loop_not_called_when_project_missing(self):
        ...
        with (
            patch("ralph.cli._assert_project_exists", side_effect=SystemExit(1)),
            patch("ralph.cli.Runner") as MockRunner,
        ):
